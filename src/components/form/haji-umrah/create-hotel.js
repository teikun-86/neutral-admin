...
export const CreateHotel = ({
    ...
    const { data, errors, setErrors, handleChange, reset } = useForm({
        location_1: "Madinah",
        location_2: "Mekkah",
        first_check_in_at: null,
        first_check_out_at: null,
    ...
    const handleSubmit = async (e) => {
        ...
        await axios.post("/hajj-umrah/hotels/store", transformed).then(res => {
            setLoading(false)
            ...
            setLoading(false)
            toast.error("Failed to create hotel. Please try again later.")
    ...
    return (
        <Modal size="lg" id="createHajiUmrahHotelModal" static>
            {
                loading && (
                        ...
                        <div className="w-full p-2">
                            <div className="mb-3">
                                <Combobox value={data.company} nullable onChange={(val) => handleChange('company', val)}>
                                    <Combobox.Input placeholder="Search company" onChange={(e) => setQuery(e.target.value)} label="Company" displayValue={(comp) => comp !== null ? `${comp.name} (PPIU: ${comp.ppiu_number})` : "No Company"} showButton />
