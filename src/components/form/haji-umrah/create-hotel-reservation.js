...
export const CreateHotelReservation = ({ refetch }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        await axios.post("/hajj-umrah/hotels/reservations/store", transformed).then(res => {
            toast.success("Successfully created hotel reservation")
            reset()
            refetch()
            setModalState(null)
        }).catch(err => {
            setLoading(false)
            toast.error("Failed to create hotel reservation")
    ...
    return (
        <Modal id="createHotelReservationModal" size="lg" static>
            {
                loading && (
            ...
            <Modal.Header>
                <Modal.Title>Create Hotel Reservation</Modal.Title>
            </Modal.Header>
            <form onSubmit={handleSubmit}>
                <Modal.Body className="text-start">
                    ...
                    <div className="w-full flex flex-wrap">
                        ...
                        <div className="w-full xl:w-1/2 p-2">
                            <div className="w-full mb-3">
                                <Input type="number" min={1} value={data.amount} name="amount" label="Amount" onChange={(e) => handleChange('amount', e.target.value)} max={data.hotel.packages_left ?? 0} />
