...
export const UpdatePackage = ({ pkg, setPkg, refetch }) => {
    const [modalOpenState, setModalState] = useRecoilState(modalState)
    const [loading, setLoading] = useState(false)
    const [pack, setPack] = useState(pkg)
    ...
    const { data, handleChange, reset, errors, setErrors } = useForm({
        flight: pack.flight,
        hotel: pack.hotel,
        packages_available: pack.packages_available,
        price_per_package: pack.price_per_package,
        seats_per_package: pack.seats_per_package,
        hotels_per_package: pack.hotels_per_package,
    ...
    const handleSubmit = async (e) => {
        ...
        setErrors({})
        setLoading(true)
        await axios.post("/hajj-umrah/packages/update", {
            id: pack.id,
    ...
    return (
        <Modal id="updatePackageModal" size="lg" static>
            {
                loading && (
